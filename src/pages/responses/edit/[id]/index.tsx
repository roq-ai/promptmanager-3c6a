...
import { useRoqClient, useResponseFindFirst } from 'lib/roq';
...
function ResponseEditPage() {
  ...
  const { data, error, isLoading, mutate } = useResponseFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ResponseInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.response.update({
        data: values as RoqTypes.response,
        where: {
          id,
        },
      });
