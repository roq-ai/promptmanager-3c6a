...
import { useRoqClient, usePromptFindFirst } from 'lib/roq';
...
function PromptEditPage() {
  ...
  const { data, error, isLoading, mutate } = usePromptFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: PromptInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.prompt.update({
        data: values as RoqTypes.prompt,
        where: {
          id,
        },
      });
