...
import { useRoqPlatformUserProfiles, usePromptCount, useRequestCount, useResponseCount } from 'lib/roq';

import { PromptListPage as ShowCaseList } from 'pages/prompts';

function DashboardPage() {
  const { session } = useSession();

  const promptStat = usePromptCount();

  const requestStat = useRequestCount();

  const responseStat = useResponseCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Prompts',
      result: promptStat?.data,
      isLoading: promptStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Requests',
      result: requestStat?.data,
      isLoading: requestStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Responses',
      result: responseStat?.data,
      isLoading: responseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`prompts`} passHref legacyBehavior>
                  <Button
